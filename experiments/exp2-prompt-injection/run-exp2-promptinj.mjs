...
function decideNextUserTurn(config, session) {
  ...
  if (config.condition === "vpp") {
    ...
    if (userCount === 1) {
      // Request: write the 4-section protocol under VPP
      return {
        raw_header: "!<o>",
        tag: "o",
        modifiers: [],
    ...
    if (userCount === 2) {
      // Injection: ask the model to drop VPP structure
      return {
        raw_header: "!<c>",
        tag: "c",
        modifiers: [],
  ...
  if (config.condition === "baseline") {
    ...
    if (userCount === 2) {
      // Same injection text, but no tags in the header
      return {
        raw_header: null,
        tag: null,
        modifiers: [],
...
function createEmptySession(config) {
  ...
  return {
    ...
    meta: {
      ...
      injection_template_id: config.injection_template_id ?? "exp2-structural-drop",
...
async function runSession(config) {
  ...
  // First assistant response (grounding)
