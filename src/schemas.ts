...
export const vppStateSchema = z.object({
  ...
  }).describe("The active VPP locus."),
  cycle: z.union([z.literal(1), z.literal(2), z.literal(3)]).describe("The active critique cycle, capped at three."),
  tag_counts: z.object({
  ...
  }).describe("Conversation-global assistant-tag counters that continue across loci."),
  closed: z.boolean().describe("Whether the preceding response used the final o_f tag.")
...
const parsedCommandSchema = z.object({
  ...
  tag: z.enum(["g", "q", "o", "c", "o_f", "e", "e_o"]).nullable().describe("The parsed user command tag, or null when invalid."),
...
export const prepareInputSchema = z.object({
  ...
  state: vppStateSchema.nullish().describe("Prior client-carried state; omit only for a new conversation."),
...
export const formatInputSchema = z.object({
  prepared_turn: preparedTurnSchema.describe("Unmodified result from vpp_prepare_turn."),
...
export const validateExchangeInputSchema = z.object({
  ...
  state: vppStateSchema.nullish().describe("State before this exchange; omit only for the first exchange."),
...
export const validateTranscriptInputSchema = z.object({
  ...
  initial_state: vppStateSchema.nullish().describe("Optional state immediately before the supplied transcript."),
