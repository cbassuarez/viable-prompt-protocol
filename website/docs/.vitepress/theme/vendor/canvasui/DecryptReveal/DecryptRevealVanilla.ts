export interface DecryptRevealOptions {
  ...
  smoothing?: number;
  /** Optional static rasterizer used when the experimental HTML-in-canvas API is unavailable. */
  fallbackPaint?: ((
...
export function createDecryptReveal(
  ...
  function syncCanvasSize() {
    ...
    } else if (sourceCtx && typeof config.fallbackPaint === "function") {
      sourceCtx.setTransform(1, 0, 0, 1, 0, 0);
      sourceCtx.clearRect(0, 0, source.width, source.height);
      config.fallbackPaint(sourceCtx, cssWidth, cssHeight, dpr);
      contentDirty = true;
    }
    cellsDirty = true;
  }
  ...
  function render() {
    ...
    const hasSource = htmlInCanvas || typeof config.fallbackPaint === "function";
    gl!.uniform1f(u.uCrisp, reducedMotion || !hasSource ? 1 : 0);
    gl!.bindFramebuffer(gl!.FRAMEBUFFER, null);
  ...
  function frame(now: number) {
    ...
    if (
      ...
      (
        reducedMotion ||
        (!htmlInCanvas && typeof config.fallbackPaint !== "function") ||
        !churning
      )
    ) {
