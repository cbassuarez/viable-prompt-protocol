...
Exp1b tests whether Viable Prompt Protocol (VPP) can be instantiated and retained **purely from user instructions**, without any system-level header snippet, and whether the protocol appears “in the wild” when a user simply types `!<q>` with no explanation.
